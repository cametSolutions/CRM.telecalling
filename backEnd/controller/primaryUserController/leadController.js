...
export const GetallselectedproductFollowup = async (req, res) => {
  try {
    ...
    const followupLeads = []
    console.log("seecteddforll", selectedfollowup)

    for (const lead of selectedfollowup) {
      // if product filter exists, make sure this lead has that product in leadFor
      // if (
      //   productObjectId &&
      //   !Array.isArray(lead.leadFor)?.some(
      //     (lf) =>
...
export const GetallproductwiseReport = async (req, res) => {
  try {
    ...
    const re = await LeadMaster.aggregate([
      ...
      // 2️⃣ Keep only followup allocations (task allocated to a staff)
      {
        $match: {
          "activityLog.taskallocatedTo": { $exists: true, $ne: null },
          "activityLog.taskTo": "followup",
      ...
      // 3️⃣ Group by lead + staff to get one row per lead per staff
      {
        $group: {
          _id: {
            leadId: "$leadId",
          ...
          netAmount: { $first: "$netAmount" }, // lead-level net amount
        },
      },

      // 4️⃣ Compute lead-level status flags
      {
        $addFields: {
          isConverted: {
            $cond: [
      ...
      // 5️⃣ Pending = neither converted nor lost
      {
        $addFields: {
          isPending: {
            $cond: [
      ...
      // 6️⃣ Aggregate per staff × branch (staff-wise stats)
      {
        $group: {
          _id: {
            staffId: "$_id.staffId",
