...
//     if (checkLicense && !isSameEditingLicense) {
//       setLicenseAvailable(false)
//     } else {
//       setLicenseAvailable(true)
//     }
...
//                 "primaryproduct" && String(item?.licensenumber || "").trim()
//           )
//           .map((item) => String(item.licensenumber))
//       )
//     ]
//   }, [tableData])
//   const getBranchesForCompany = (productId, companyId) => {
...
//     const companies = getCompaniesForProduct(selectedOption?.value)
...
//     setPopupType(item?.productorservicetype || "Primaryproduct")
//     setEditIndex(index)
...
//     setBranchOptions(getBranchesForCompany(item?.productid, item?.company_id))
...
//     // const row = {
//     //   companyid: values?.companyName?.value || "",
...
//     //   taggedLicenses: values?.taggedLicenses || []
//     // }
// console.log(values)
//     const row = {
...
//       productName: values?.productName?.label || "",
//       licensenumber:
//         popupType === "Primaryproduct" ? values?.licensenumber || "" : "",
//       softwareTrade: values?.softwareTrade || "",
//       applicationDate: values?.applicationDate || "",
//       nextDue: values?.nextDue || "",
//       noofusers: values?.noofusers || "",
...
//   console.log(filteredOptionsByType)
...
//                   required: "RegistrationType is required"
//                 })}
//                 className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-2 sm:text-sm outline-none cursor-pointer"
//               >
//                 <option value="">Select RegistrationType</option>
//                 <option value="unregistered">UnregisteredConsumer</option>
//                 <option value="regular">Regular</option>
//               </select>
//               {errors.registrationType && (
//                 <span className="mt-2 text-sm text-red-600">
...
//               Product Details
//             </h1>
//             <hr className="my-2 border-gray-300 border-2" />

//             <div className="mt-6 rounded-sm border border-gray-300 bg-[#f3f3f3] p-6">
...
//                               className={`relative h-[126px] w-[126px] rounded-full border text-center shadow-sm ${
//                                 isDeactive
//                                   ? "bg-[#f4cabb] border-[#efc0b2]"
//                                   : "bg-[#bfd8ac] border-[#b2cf9b]"
//                               }`}
//                             >
//                               <div className="flex h-full flex-col items-center justify-center px-3">
//                                 <p className="text-[11px] font-medium text-black leading-4">
//                                   {item?.productName}
//                                 </p>
...
//                               type="button"
//                               onClick={() => handleEdit(item, actualIndex)}
//                               className="relative h-[126px] w-[126px] rounded-full border border-[#eedb8a] bg-[#f1dc88] text-center shadow-sm"
//                             >
//                               {isTagged && (
//                                 <span className="absolute left-7 top-6 flex h-6 w-6 items-center justify-center bg-white/80 text-gray-500 shadow-sm">
//                                   📎
//                                 </span>
//                               )}
...
//                                   Due — {formatDateToDDMMYYYY(item?.nextDue || "-")}
//                                 </p>
//                                 <p className="mt-2 text-[11px] text-black">
...
//                   <div className="mb-14">
//                     <p className="text-[13px] font-semibold underline">
//                       Primary Products
//                     </p>
//                     <button
//                       type="button"
//                       onClick={() => openAddPopup("Primaryproduct")}
//                       className="mt-3 text-[13px] text-black hover:underline"
//                     >
//                       Add
//                     </button>
//                   </div>
...
//                         <div className="max-h-40 overflow-y-auto rounded border px-3 py-2">
...
//             <button
//               type="submit"
//               className="flex items-center justify-center bg-green-500 text-white py-2 px-4 rounded shadow-xl hover:bg-green-600 disabled:bg-green-400 disabled:cursor-not-allowed"
//               disabled={isSubmitting}
//             >
//               {isSubmitting
//                 ? "Submitting..."
...
//   useEffect(() => {
//     if (productError) {
//       toast.error(productError?.response?.data?.message || "Something went wrong!")
//     }
//   }, [productError])
...
//       const selectedData =
//         customer?.selected?.map((sel) => ({
//           companyid: sel?.companyid?.id,
//           companyName: sel?.companyid?.companyName,
//           branchid: sel?.branchid?.id,
//           branchName: sel?.branchid?.branchName,
//           productid: sel?.productid?.id,
...
//             sel?.tvuexpiryDate,
//           noofusers: sel?.noofusers,
//           amount: sel?.amount || sel?.productAmount || sel?.amcAmount || sel?.tvuAmount,
//           isActive: sel?.isActive || "Running",
//           productorservicetype: sel?.productorservicetype,
//           taggedLicenses: sel?.taggedLicenses || sel?.licenseNumbers || []
//         })) || []
...
//     const productOption =
//       productOptions.find((p) => p.value === item?.productid) || {
//         label: item?.productName,
...
//       productName: values?.productName?.label,
//       licensenumber:
//         popupType === "Primaryproduct" ? values?.licensenumber : values?.licensenumber,
//       softwareTrade: values?.softwareTrade,
//       applicationDate: values?.applicationDate,
//       nextDue: values?.nextDue,
//       noofusers: values?.noofusers,
...
//             <div>
//               <p className="text-sm font-medium text-indigo-600">Customer Master</p>
//               <h1 className="mt-1 text-2xl font-bold tracking-tight text-slate-900">
//                 {process === "edit" ? "Edit Customer" : "Customer Registration"}
//               </h1>
//               <p className="mt-1 text-sm text-slate-500">
//                 Add customer details and map products or services without sidebar layout.
//               </p>
//             </div>
...
//                   Basic profile, contact, location and registration info.
//                 </p>
//               </div>
//             </div>
...
//                 <select {...register("partner")} className={inputClass}>
//                   <option value="">Select Partner</option>
//                   {partner?.map((partnr, index) => (
//                     <option key={index} value={partnr.id}>
//                       {partnr.partner}
...
//                 <p className="text-sm text-slate-500">
//                   Manage primary products and additional services linked to the customer.
//                 </p>
//               </div>
...
//                   <h3 className="text-base font-semibold text-slate-900">Primary Products</h3>
//                   <span className="rounded-full bg-emerald-100 px-3 py-1 text-xs font-medium text-emerald-700">
//                     {primaryProducts.length} items
//                   </span>
//                 </div>
...
//                           color={isDeactive ? "red" : "green"}
//                           subtitle={item?.licensenumber}
//                           footer={`Due ${formatDateToDDMMYYYY(item?.nextDue)}`}
//                           status={isDeactive ? "De Active" : item?.isActive}
//                           onEdit={handleEdit}
...
//                     })
//                   ) : (
//                     <EmptyState text="No primary products added." />
//                   )}
//                 </div>
//               </div>
...
//                     {additionalServices.length} items
//                   </span>
//                 </div>
...
//                     })
//                   ) : (
//                     <EmptyState text="No additional services added." />
//                   )}
//                 </div>
//               </div>
//             </div>
...
//                 </h3>
//                 <p className="text-sm text-slate-500">
//                   Fill the details and save the item to customer mapping.
//                 </p>
//               </div>
...
//                   className={inputClass}
//                   placeholder={
//                     popupType === "Primaryproduct" ? "Enter license number" : "Optional"
//                   }
//                   onChange={(e) => {
//                     setValue("licensenumber", e.target.value)
//                     clearErrors("licensenumber")
...
//               <Field label="Software Trade">
//                 <select {...register("softwareTrade")} className={inputClass}>
//                   <option value="">Select Software Trade</option>
//                   {softwareTrades.map((trade, index) => (
//                     <option key={index} value={trade}>
//                       {trade}
...
//                 <input type="date" {...register("nextDue")} className={inputClass} />
//               </Field>

//               <Field label="No of Quantity / Users">
//                 <input
//                   type="number"
//                   {...register("noofusers")}
//                   className={inputClass}
...
//                   <div className="max-h-44 overflow-y-auto rounded-2xl border border-slate-200 bg-slate-50 p-4">
//                     {primaryLicenseOptions.length > 0 ? (
...
//                             <label
//                               key={licenseNo}
//                               className="flex items-center gap-3 rounded-xl bg-white px-3 py-2 text-sm text-slate-700 ring-1 ring-slate-200"
//                             >
//                               <input
//                                 type="checkbox"
//                                 checked={checked}
...
// }) => {
//   const colorMap = {
//     green: "border-emerald-200 bg-emerald-100/80",
//     red: "border-rose-200 bg-rose-100/80",
//     yellow: "border-amber-200 bg-amber-100/80"
//   }
...
//         className={`relative flex h-36 w-36 flex-col items-center justify-center rounded-full border text-center shadow-sm transition hover:scale-[1.02] ${colorMap[color]}`}
//       >
//         <p className="px-4 text-xs font-semibold leading-4 text-slate-900">
//           {item?.productName}
//         </p>
//         <p className="mt-2 px-4 text-[11px] text-slate-700">{subtitle}</p>
//         <p className="mt-2 px-4 text-[11px] text-slate-700">{footer}</p>
//         <p className="mt-2 px-4 text-[11px] font-semibold text-slate-900">
//           {status}
//         </p>
//       </button>
...
//           type="button"
//           onClick={() => onEdit(item, actualIndex)}
//           className="rounded-full bg-white p-2 text-emerald-600 shadow"
//         >
//           <FaEdit size={10} />
//         </button>
//         <button
...
//   <div className="flex min-h-[140px] w-full items-center justify-center rounded-2xl border border-dashed border-slate-300 bg-white text-sm text-slate-400">
//     {text}
//   </div>
// )
...
//       {showProductPopup && (
//         <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 px-4">
//           <div className="w-full max-w-3xl rounded-[20px] bg-white shadow-2xl">
//             <div className="flex items-center justify-between border-b border-[#edf1f7] px-5 py-4">
//               <div>
//                 <h3 className="text-[16px] font-semibold text-[#162033]">
//                   {popupType === "Primaryproduct"
//                     ? "Primary Product"
//                     : "Additional Service"}
//                 </h3>
...
//                 type="button"
//                 onClick={closePopup}
//                 className="rounded-full p-2 text-[#7f8aa3] hover:bg-[#f4f7fb]"
//               >
//                 <FaTimes />
//               </button>
//             </div>
...
//                 <select {...register("softwareTrade")} className={popupInputClass}>
//                   <option value="">Select Software Trade</option>
//                   {softwareTrades.map((trade, index) => (
//                     <option key={index} value={trade}>
//                       {trade}
...
//                 <select {...register("isActive")} className={popupInputClass}>
//                   <option value="Running">Active</option>
//                   <option value="Deactive">Deactive</option>
//                 </select>
//               </PopupField>
...
//                   <div className="max-h-44 overflow-y-auto rounded-[12px] border border-[#e7ebf4] bg-[#fafcff] p-3">
//                     {primaryLicenseOptions.length > 0 ? (
...
//                             <label
//                               key={licenseNo}
//                               className="flex items-center gap-2 rounded-lg border border-[#edf1f7] bg-white px-3 py-2 text-[12px] text-[#4f5d78]"
//                             >
//                               <input
//                                 type="checkbox"
//                                 checked={checked}
...
//                       </div>
//                     ) : (
//                       <p className="text-[12px] italic text-[#96a0b5]">
//                         No primary product license numbers available.
//                       </p>
//                     )}
//                   </div>
...
//                 type="button"
//                 onClick={closePopup}
//                 className="rounded-lg border border-[#e4e9f2] px-4 py-2 text-[13px] text-[#5c6981] hover:bg-[#f8fafc]"
//               >
//                 Cancel
//               </button>
//               <button
//                 type="button"
//                 onClick={savePopupData}
//                 className="rounded-lg bg-[#2f80ed] px-4 py-2 text-[13px] font-semibold text-white hover:bg-[#246cd0]"
//               >
//                 {editIndex !== null ? "Update" : "Save"}
//               </button>
//             </div>
...
//   return (
//     <div className="min-h-screen bg-[#f6f8fc] px-3 py-6 md:px-6">
//       <div className="mx-auto max-w-[1180px]">
//         <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
//           <div className="rounded-[20px] border border-[#edf1f7] bg-white p-4 shadow-[0_8px_30px_rgba(15,23,42,0.05)] md:p-5">
//             <div className="mb-4 flex items-center justify-between">
...
//             <section className="rounded-[18px] border border-[#edf1f7] bg-white shadow-[0_6px_24px_rgba(15,23,42,0.05)]">
//               <div className="flex items-center justify-between border-b border-[#eef2f7] px-4 py-4">
//                 <div className="flex items-center gap-3">
//                   <div className="flex h-8 w-8 items-center justify-center rounded-full bg-[#eef5ff] text-[#2f80ed]">
//                     <FaCube size={14} />
//                   </div>
//                   <h3 className="text-[14px] font-semibold text-[#182235]">
//                     Primary Products
//                   </h3>
//                 </div>
...
//                   type="button"
//                   onClick={() => openAddPopup("Primaryproduct")}
//                   className="inline-flex items-center gap-2 text-[12px] font-medium text-[#2f80ed] hover:underline"
//                 >
//                   Add Product
...
//                           actualIndex={actualIndex}
//                           variant={isDeactive ? "danger" : "success"}
//                           topBadgeIcon={<FaCube size={11} />}
//                           line1={item?.productName}
//                           line2={item?.licensenumber}
//                           line3={item?.nextDue ? formatDateToDDMMYYYY(item?.nextDue) : ""}
//                           line4={isDeactive ? "De Active" : ""}
//                           onEdit={handleEdit}
//                           onDelete={handleDelete}
//                         />
...
//                   </div>
//                 ) : (
//                   <div className="flex h-[160px] items-center justify-center rounded-[14px] border border-dashed border-[#e7ebf4] bg-[#fbfcff] text-[12px] text-[#8a95ab]">
//                     No primary products added.
//                   </div>
//                 )}
//               </div>
...
//                   </div>
//                 ) : (
//                   <div className="flex h-[160px] flex-col items-center justify-center rounded-[14px] border border-dashed border-[#e7ebf4] bg-[#fbfcff] text-center">
//                     <div className="mb-3 flex h-12 w-12 items-center justify-center rounded-full bg-[#f2f4f8] text-[#9ca8be]">
//                       <FaBoxOpen size={15} />
//                     </div>
//                     <p className="text-[13px] text-[#76839d]">
//                       No additional services added.
//                     </p>
//                     <p className="mt-1 text-[11px] text-[#a0abc0]">
//                       Click "Add Service" to get started.
...
//         type="button"
//         onClick={() => onEdit(item, actualIndex)}
//         className={`relative flex h-[128px] w-[128px] flex-col items-center justify-center rounded-full border text-center shadow-sm transition hover:scale-[1.02] ${variantClass}`}
//       >
//         <div className="mb-2 flex h-8 w-8 items-center justify-center rounded-full bg-white/80 text-[#4e5a72] shadow-sm">
//           {topBadgeIcon}
//         </div>

//         <p className="px-3 text-[11px] font-semibold leading-4 text-[#1e293b]">
//           {line1}
//         </p>

//         {line2 ? (
//           <p className="mt-1 px-3 text-[10px] leading-4 text-[#4b5563]">
//             {line2}
//           </p>
//         ) : null}
...
//         {line4 ? (
//           <p className="mt-1 px-3 text-[10px] font-semibold leading-4 text-[#d35c5c]">
//             {line4}
//           </p>
//         ) : null}
//       </button>
...
// }) => {
//   return (
//     <div className="group relative rounded-[14px] border border-[#eef2f7] bg-[#fbfcff] p-3 transition hover:border-[#d9e6ff]">
//       <button
//         type="button"
//         onClick={() => onEdit(item, actualIndex)}
...
//       >
//         <div className="flex items-start gap-3">
//           <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-[#fff6d9] text-[#d6a400]">
//             <FaBoxOpen size={14} />
//           </div>

//           <div className="min-w-0 flex-1">
//             <div className="flex items-start justify-between gap-2">
//               <p className="text-[13px] font-semibold text-[#182235]">
//                 {item?.productName}
//               </p>
//               <span className="rounded-full bg-[#eef6ff] px-2 py-1 text-[10px] font-medium text-[#2f80ed]">
//                 Service
//               </span>
//             </div>

//             {item?.amount ? (
//               <p className="mt-1 text-[12px] text-[#4f5d78]">
//                 Amount - Rs. {item.amount}
//               </p>
//             ) : null}

//             {item?.nextDue ? (
//               <p className="mt-1 text-[12px] text-[#d45b5b]">
//                 Due - {formatDateToDDMMYYYY(item?.nextDue)}
//               </p>
//             ) : null}

//             {Array.isArray(item?.taggedLicenses) && item.taggedLicenses.length > 0 ? (
//               <p className="mt-1 text-[11px] text-[#7a879f]">
//                 Tagged: {item.taggedLicenses.join(", ")}
//               </p>
//             ) : (
//               <p className="mt-1 text-[11px] text-[#a0abc0]">
//                 No tagged licenses
...
//             <div className="rounded-[16px] border border-[#e9edf5] bg-white shadow-[0_8px_24px_rgba(15,23,42,0.04)]">
//               <div className="flex items-center justify-between border-b border-[#f1f4f8] px-4 py-3">
//                 <div className="flex items-center gap-3">
//                   <div className="flex h-6 w-6 items-center justify-center rounded-md bg-[#eef6ff] text-[#2f80ed]">
//                     <FaBoxOpen size={11} />
//                   </div>
//                   <h3 className="text-[13px] font-semibold text-[#1b2437]">
//                     Primary Products
...
//                   type="button"
//                   onClick={() => openAddPopup("Primaryproduct")}
//                   className="inline-flex items-center gap-2 text-[11px] font-medium text-[#2f80ed] hover:underline"
//                 >
//                   Add Product
...
//               <div className="min-h-[190px] px-4 py-5">
//                 {primaryProducts.length > 0 ? (
//                   <div className="flex flex-wrap gap-5">
//                     {primaryProducts.map((item) => {
//                       const actualIndex = tableData.findIndex((x) => x === item)
...
//                   </div>
//                 ) : (
//                   <div className="flex h-[145px] items-center justify-center rounded-[12px] border border-dashed border-[#e7ebf4] bg-[#fbfcff] text-[12px] text-[#8a95ab]">
//                     No primary products added.
//                   </div>
//                 )}
//               </div>
...
//                   </div>
//                 ) : (
//                   <div className="flex h-[145px] flex-col items-center justify-center rounded-[12px] bg-white text-center">
//                     <div className="mb-3 flex h-14 w-14 items-center justify-center rounded-full bg-[#f3f5f9] text-[#a0a8b8]">
//                       <FaBoxOpen size={18} />
//                     </div>
//                     <p className="text-[13px] text-[#6f7c95]">
//                       No additional services added.
//                     </p>
//                     <p className="mt-1 text-[12px] text-[#9aa6bc]">
//                       Click 'Add Service' to get started.
//                     </p>
//                   </div>
//                 )}
...
//   const bubbleClass =
//     variant === "danger"
//       ? "border-[#f6cdcd] bg-[radial-gradient(circle_at_top,_#fff5f5_0%,_#ffdede_58%,_#ffd3d3_100%)]"
//       : "border-[#d8efc9] bg-[radial-gradient(circle_at_top,_#f7fff3_0%,_#def7cb_58%,_#d4f0bf_100%)]"

//   const iconColor = variant === "danger" ? "text-[#de4f4f]" : "text-[#46a84d]"
...
//         type="button"
//         onClick={() => onEdit(item, actualIndex)}
//         className={`flex h-[150px] w-[150px] flex-col items-center justify-center rounded-full border text-center shadow-sm transition hover:scale-[1.02] ${bubbleClass}`}
//       >
//         <div className={`mb-3 flex h-11 w-11 items-center justify-center rounded-full bg-white/55 ${iconColor}`}>
//           <FaBoxOpen size={18} />
//         </div>

//         <p className="px-3 text-[11px] font-semibold leading-4 text-[#1c2231]">
//           {item?.productName}
//         </p>

//         <p className="mt-1 px-3 text-[10px] leading-4 text-[#263247]">
//           {item?.licensenumber}
//         </p>

//         {String(item?.isActive).toLowerCase() === "deactive" && (
//           <p className="mt-1 px-3 text-[10px] font-medium text-[#d95c5c]">
//             De Active
//           </p>
//         )}
//       </button>
...
// }) => {
//   return (
//     <div className="group relative rounded-[12px] border border-[#edf1f7] bg-[#fbfcff] p-3">
//       <button
//         type="button"
//         onClick={() => onEdit(item, actualIndex)}
//         className="w-full text-left"
//       >
//         <div className="flex items-start gap-3">
//           <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-[#f3f5f9] text-[#98a1b2]">
//             <FaBoxOpen size={13} />
//           </div>

//           <div className="min-w-0 flex-1">
//             <p className="text-[12px] font-semibold text-[#1b2437]">
//               {item?.productName}
//             </p>

//             {item?.amount ? (
//               <p className="mt-1 text-[11px] text-[#66748d]">
//                 Amount - Rs. {item.amount}
//               </p>
//             ) : null}
...
//             {Array.isArray(item?.taggedLicenses) && item.taggedLicenses.length > 0 ? (
//               <p className="mt-1 text-[10px] text-[#8c97ab]">
//                 Tagged: {item.taggedLicenses.join(", ")}
//               </p>
//             ) : null}
//           </div>
