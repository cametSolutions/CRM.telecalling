 ...
 *     onSuccess={(saved) => {
 *       // e.g. refresh the banner / list with `saved`
 *       setOpen(false);
 *     }}
 *   />
 *
 * Replace `submitAnnouncement` with your real endpoint.
 */
...
export default function AnnouncementModal({ open, onClose, onSuccess }) {
console.log("Hh")
...
console.log("hh")
  ...
  const handleSubmit = async (e) => {
    ...
    setError("");
    try {
      const saved = await submitAnnouncement({ title: title.trim() });
      onSuccess?.(saved);
      setTitle("");
    } catch (err) {
