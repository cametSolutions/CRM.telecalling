export default function ReportTable({
  ...
  const PRODUCTWISE_AMOUNT_KEY = {
    "Total Leads": "totalNetAmount",
    Converted: "totalConvertedAmount",
  ...
  const rawColumns = sourceHeaders.map((header) => {
    ...
    if (isFollowup) {
      ...
      if (header === "Upcoming") accessor = "future"
      if (header === "New Lead") accessor = "neverFollowup"
    }
  ...
  const visibleColumns = columns.filter((col) => {
    ...
    if (isProductWise) {
      // Always hide these internal IDs
  ...
  const getCellType = (col) => {
    ...
    // Staff cell - only clickable in staff mode without drill-down
    if (
      col.type === "staff" &&
      mode === "staff" &&
      !drillDown &&
    ...
    // Product cell - show as primary in product mode
    if (col.type === "product" && mode === "product" && !drillDown)
      return "product"
          ...
          <div>
            ...
            >
              {drillDown && selectedStaff
                ...
                : `${effectiveData.length} ${effectiveData.length === 1 ? "record" : "records"} • ${mode === "staff" ? "Staff View" : "Product View"}`}
            </div>
          </div>
        </div>
                  ...
                  return (
                    ...
                    >
                      {visibleColumns.map((col, colIdx) => {
                        ...
                        /* ── PRODUCT CELL (when in product mode) ── */
                        if (type === "product") {
                          const name = String(value || "")
                          return (
                        ...
                        if (type === "metric") {
                          ...
                          const amountKey = isProductWise
                            ? PRODUCTWISE_AMOUNT_KEY[col.header]
                            : null
                          const amountVal =
