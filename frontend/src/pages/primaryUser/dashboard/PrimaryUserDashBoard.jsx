...
<div className="min-h-full bg-[#ADD8E6] overflow-x-hidden">
  ...
  <div
    ref={headerRef}
    className="flex flex-col sticky top-0 z-40 md:flex-row md:flex-wrap md:justify-center md:gap-4 space-y-2 md:space-y-0 px-3 md:px-4 py-3 border-b shadow-lg bg-[#023e7d]"
  >
    {(cards || []).map((item, index) => {
      const Icon = item.icon
      return (
        <Link to={item.to} key={index} className="w-full md:w-60">
          <div className="w-full h-12 md:h-16 font-semibold shadow-md p-3 rounded-md flex items-center gap-3 bg-gradient-to-r from-white via-sky-50 to-sky-100 hover:shadow-lg hover:scale-[1.02] transform transition-all duration-200 border border-sky-100">
            <div className="bg-emerald-600 p-2 rounded-full text-white shadow-sm shrink-0">
              <Icon className="md:text-xl" />
            </div>
            <span className="text-slate-800 tracking-wide text-xs md:text-sm truncate">
              {item.label?.toUpperCase()}
            </span>
          </div>
        </Link>
  ...
  {/* Main Content — CSS Grid instead of nested flex: each column gets an
      explicit fraction of the row, so a column can never silently collapse
      to near-zero width the way nested flex-1 sometimes can under certain
      wrapping/overflow combinations. */}
  <div className="grid grid-cols-1 lg:grid-cols-2 gap-3 md:gap-6 m-2 p-2 md:p-4">
    {/* LEFT SIDE */}
    <div className="flex flex-col gap-3 md:gap-6 min-w-0">
      {/* Top Row - Leave and Birthday */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-6">
        {/* Leave List */}
        <div className="p-4 rounded-lg shadow-md bg-white/90 border border-orange-200 min-h-[160px]">
          <h2 className="font-semibold text-base md:text-lg mb-3 text-orange-700 flex items-center justify-between">
            <span>Today's Leave</span>
            <span className="text-[11px] text-orange-500">
              {leaveList?.length || 0} staff
        ...
        {/* Birthday List */}
        <div className="p-4 rounded-lg shadow-md bg-white/90 border border-pink-200 min-h-[160px]">
          <h2 className="font-semibold text-base md:text-lg mb-3 text-pink-700 flex items-center justify-between">
            <span>Birthdays</span>
            <span className="text-[11px] text-pink-500">
              {birthdays?.length || 0} this month
    ...
    {/* RIGHT SIDE — also grid, so Monthly Holidays / Achievers each get a
        guaranteed, explicit column width instead of relying on flex-1
        inside a flex-row inside a flex-row (which is what was collapsing). */}
    <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-6 min-w-0">
      {/* Monthly Holidays */}
      <div className="p-4 rounded-lg shadow-md bg-white/90 border border-emerald-200 min-h-[220px] min-w-0">
        <h2 className="font-semibold text-base md:text-lg mb-3 text-emerald-700">
          Monthly Holidays
        </h2>
        {currentyearholydays && currentyearholydays.length > 0 ? (
          <div className="space-y-2 text-sm">
            {[...currentyearholydays]
              ...
              .map((holiday) => (
                <div
                  key={holiday._id}
                  className="flex items-center justify-between gap-2 border-b border-emerald-50 pb-1 last:border-none"
                >
                  <span className="font-medium text-emerald-700 truncate">
                    {holiday?.customTextInput}
                  </span>
                  <span className="text-sky-700 text-xs shrink-0">
                    {holiday?.holyDate
                      ? new Date(holiday.holyDate).toLocaleDateString(
                          "en-IN",
      ...
      {/* Achievers & Announcements */}
      <div className="p-4 rounded-lg shadow-md flex flex-col gap-4 group border border-purple-200 relative min-h-[220px] min-w-0 bg-white/95">
        {user?.role === "Admin" && (
          <div className="md:absolute md:left-1/2 md:top-1/4 md:-translate-x-1/2 md:-translate-y-1/2 md:inset-auto w-[90%] md:w-full border border-purple-200 bg-white rounded-lg shadow-xl p-4 text-sm hidden group-hover:block z-30 m-4 md:m-0 max-h-[90vh] md:max-h-96 overflow-y-auto">
            <h3 className="font-bold text-center text-purple-700 mb-2">
              Update Achievers
            </h3>
                  ...
                  {(allStaffs || []).map((staff) => (
                    ...
                    >
                      <input
                        type="checkbox"
                        className="mr-2 accent-purple-600"
                        checked={selectedQuarterlyStaffs?.[staff._id] || false}
                        onChange={() => handleQuarterlyStaffToggle(staff._id)}
                      />
                      {staff.name?.toUpperCase()}
                    </label>
                  ))}
        ...
        {/* Quarterly + Yearly Achievers Display */}
        <div className="rounded flex-1 w-full min-w-0">
          <h3 className="font-bold text-lg mb-1 text-purple-700">
            Quarterly Achiever
          </h3>
          <p className="text-center break-words text-transparent bg-clip-text bg-gradient-to-r from-purple-600 via-pink-500 to-red-500 font-bold text-lg drop-shadow-sm mb-2">
            {quarterlyTitle || "—"}
          </p>

          {acheivementlist?.quarterlyachiever?.length > 0 ? (
            <div className="space-y-2 text-sm">
              {acheivementlist.quarterlyachiever.map((achiever, index) => (
                <div
                  key={index}
                  className="flex justify-between items-center gap-2 p-2 bg-purple-50 rounded-lg border border-purple-100"
                >
                  ...
                  {achiever?.achieverId?.profileUrl?.[0] && (
                    <img
                      src={achiever.achieverId.profileUrl[0]}
                      alt={achiever?.achieverId?.name}
                      className="w-10 h-10 shrink-0 rounded-full border-2 border-purple-300 object-cover"
                    />
                  )}
                </div>
              ))}
        ...
        {/* Announcements */}
        <div className="rounded flex-1 mt-4 group relative min-w-0">
          <h3 className="font-bold text-lg mb-2 text-purple-700">
            Announcement
          </h3>
          ...
          {announcement ? (
            <div className="relative bg-yellow-100/90 p-4 rounded-lg shadow-md border border-yellow-300 mx-auto max-w-xs md:max-w-lg">
              ...
              <div className="mt-1 text-yellow-900 font-medium text-center whitespace-pre-wrap text-sm break-words">
                {announcement}
              </div>
            </div>
          ) : (
            <div className="flex flex-col items-center justify-center py-6 text-amber-700 text-sm">
              ...
              <p className="italic text-center text-xs">No announcement yet</p>
            </div>
          )}
        </div>
      </div>
