...
//     safeState?.istotal ? false : safeState?.pending ? false : true
//   )
...
//   const shouldFetch =
//     (!safeState?.staffId ||
//       (safeState?.istotal && safeState?.staffId)) &&
//     loggedUser &&
//     selectedCompanyBranch
//   console.log(shouldFetch)

//   console.log(safeState?.pending)

//   const url = shouldFetch
//     ? `/lead/getallLeadFollowUp?branchSelected=${selectedCompanyBranch}&loggeduserid=${safeState?.istotal ? safeState?.staffId : loggedUser?._id}&role=${safeState?.istotal ? safeState?.staffRole : loggedUser?.role}&pendingfollowup=${safeState?.pending ? safeState?.pending : pending}`
//     : null
// console.log(selectedCompanyBranch)
// console.log(`/lead/getallLeadFollowUp?branchSelected=${selectedCompanyBranch}&loggeduserid=${safeState?.istotal ? safeState?.staffId : loggedUser?._id}&role=${safeState?.istotal ? safeState?.staffRole : loggedUser?.role}&pendingfollowup=${safeState?.pending ? safeState?.pending : pending}`)
//   const {
//     data: loggedusersallocatedleads,
//     loading,
//     error,
...
//   } = UseFetch(url)
//   useEffect(() => {
//     if (!safeState?.branchId && !branches) return
...
//       }))
//     // console.log(users)
//     // console.log(loggeduserbranchesinproductwise)
...
//     if (!safeState?.staffId || !loggedUser || safeState?.istotal)
//       return
//     console.log(location.state)
//     const selectedbranch = safeState?.branchId
//     // console.log(selectedbranch)
...
//   console.log(branches && !safeState?.staffId)
// console.log(typeof !safeState?.staffId)
//   useEffect(() => {
// console.log("hh")
// console.log(branches)
//     if (branches && !safeState?.staffId) {
//       console.log(branches)
...
//                 setDates={setDates}
//                 dates={dates}
//                 view={safeState?.staffId ? true : false}
//               />
//             </div>
//           )}
//           {/* Filter Button */}
...
//                       setAllocatedLeads([])
//                     },
//                     show: safeState?.staffId ? false : pending === true // 👈 show only when pending is true
//                   },
//                   {
...
//                       setTableData([])
//                       setAllocatedLeads([])
//                       if (safeState?.staffId && !location.state.istotal) {
...
//                   {staffName && (
//                     <h3 className="text-base font-semibold text-gray-800 mb-2 ml-3 md:ml-5">
//                       {staffName}{" "}
...
//   const [hasOwnLeads, setHasownLeads] = useState(false)
//   const [ownFollowUp, setOwnFollowUp] = useState(
//     location?.state?.istotal ? false : location?.state?.pending ? false : true
//   )
//   const [historyList, setHistoryList] = useState([])
//   const [loggedUser, setloggedUser] = useState(null)
//   const [loggedUserBranches, setloggedUserBranches] = useState([])
...
//   // helper to guard location.state
//   const safeState = location?.state || {}
...
//   const shouldFetch =
//     (!safeState.staffId || (safeState.istotal && safeState.staffId)) &&
//     loggedUser &&
//     selectedCompanyBranch
...
//   // Initial loggedUser + branches when coming from dashboard with staffId
//   useEffect(() => {
...
//     // if we navigated with staffId, loggedUser should be that staff
...
//   // When there is no staffId in state (normal followup screen)
//   useEffect(() => {
...
//   // default selected branch (first from loggedUserBranches)
//   useEffect(() => {
//     if (
//       loggedUserBranches &&
//       loggedUserBranches.length > 0 &&
...
//     const allLeads = loggedusersallocatedleads.followupLeads || []

//     const currentDate = new Date()
//     const endDateLocal = getLocalDate(new Date(dates.endDate))
//     const currentStr = formatdate(currentDate)
//     const fulldatecurrent =
//       currentStr === endDateLocal ? currentStr : endDateLocal

//     const havenextFollowup = allLeads.filter((lead) => lead.Nextfollowup)
//     const neverfollowupedLeads = allLeads.filter(
//       (lead) => lead.neverfollowuped && lead.allocatedfollowup == false
//     )
...
//     const taskSubmittedLeads = allLeads.filter(
//       (lead) => lead.allocatedfollowup && lead.allocatedTaskClosed
//     )

//     const nonsubmittedtakleads = allLeads.filter(
//       (lead) => lead.allocatedfollowup && lead.allocatedTaskClosed === false
//     )
...
//     if (ownFollowUp) {
//       baseLeads = allLeads.filter((lead) =>
//         lead.activityLog?.some(
//           (log) =>
//             log.taskTo === "followup" &&
//             log.taskallocatedTo?._id === loggedUser._id
...
//       if (statusAllocated) {
//         // show only allocated, grouped by allocatedTo
//         const groupedallocatedleads = {}
//         nonsubmittedtakleads.forEach((lead) => {
//           const assignedTo = lead?.allocatedTo?.name || "Unassigned"
//           if (!groupedallocatedleads[assignedTo]) {
...
//         setallocatednetAmount(TotalAmount(nonsubmittedtakleads))
//       } else {
//         const ownBase = ownFollowUp
//           ? mergedall.filter((lead) =>
//               lead.activityLog?.some(
//                 (log) =>
//                   log.taskTo === "followup" &&
//                   log.taskallocatedTo?._id === loggedUser._id
...
//           setTableData(groupedData)
//         } else {
//           const Data = normalizeTableData(ownBase)
//           setTableData(Data)
//         }
//         setnetTotalAmount(TotalAmount(mergedall))
//       }
...
//       let filteredCleared = clearedLeads
//       if (ownFollowUp) {
//         filteredCleared = clearedLeads.filter((lead) =>
//           lead.activityLog?.some(
//             (log) =>
//               log.taskTo === "followup" &&
//               log.taskallocatedTo?._id === loggedUser._id
...
//       const groupedData = normalizeTableData(groupedLeads)
//       setTableData(groupedData)
//       setnetTotalAmount(TotalAmount(filteredCleared))
//     }
...
//     const lastLog =
//       item.activityLog && item.activityLog.length
//         ? item.activityLog[item.activityLog.length - 1]
//         : null
...
//     const isAllocatedToeditable = item.activityLog?.some(
//       (it) =>
//         it?.taskallocatedTo?._id === loggedUser?._id && it?.taskClosed === false
//     )
...
//           isOwner={isOwner}
//           loggedUser={loggedUser}
//           // other props you pass…
//         />
//       )}

//       {collectionupdateModal && (
//         <CollectionupdateModal
//           open={collectionupdateModal}
//           onClose={() => setcollectionUpdateModal(false)}
//           onSubmit={handleCollectionUpdate}
//           selectedLead={selectedData}
//         />
//       )}

//       {/* followup modal etc – keep your existing JSX here */}
//     </div>
//   )
// }
...
//   const safeState = location?.state || {} // NEW: guard location.state
...
//   const url= shouldFetch ? `/lead/getallLeadFollowUp?branchSelected=${selectedCompanyBranch}` +
//       `&loggeduserid=${safeState.istotal ? safeState.staffId : loggedUser._id}` +
//       `&role=${safeState.istotal ? safeState.staffRole : loggedUser.role}` +
//       `&pendingfollowup=${safeState.pending ? safeState.pending : pending}`
//     : null
...
//     const now = new Date()
//     const startDate = new Date(now.getFullYear(), now.getMonth(), 1)
//     const endDate = new Date(now.getFullYear(), now.getMonth() + 1, 0) // day 0 of next month = last day of current month
//     setDates({ startDate, endDate })
//   }, [branches, data, safeState.branchId, safeState.staffId])
// useEffect(()=>{
//   const userData = localStorage.getItem("user")
...
//     try {
//       if (!loggedUser || !selectedDocId) return // NEW: guard
//       setLoader(true)
...
//     const lastLog =
//       Array.isArray(item.activityLog) && item.activityLog.length // NEW: guard
//         ? item.activityLog[item.activityLog.length - 1]
//         : null
...
//     const isAllocatedToeditable = Array.isArray(item.activityLog) // NEW: guard
...
//               type="button"
//               onClick={() =>
//                 originalloggeduser?.role === "Admin" // NEW: optional chaining
//                   ? navigate("/admin/transaction/lead/leadEdit", {
//                       state: {
//                         leadId: item._id,
//                         isReadOnly: !isAllocatedToeditable
...
//       {/* your existing followup modal JSX here */}
//     </div>
//   )
// }
